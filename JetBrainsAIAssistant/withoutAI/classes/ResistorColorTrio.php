...
class ResistorColorTrio
{
    public function label(array $colors): string
    {
        ...
        if (isset($firstColor)) {
            $firstNum = $colorCodes[strtolower($firstColor)] * 10;
        }

        if (isset($secondColor)) {
            $secondNum = $colorCodes[strtolower($secondColor)];
        }

        if (isset($thirdColor)) {
            $multiplyBy = pow(10, $colorCodes[strtolower($thirdColor)]);
        }

        $num = ($firstNum + $secondNum) * ($multiplyBy ?? 1);
        ...
        return $num.' '.$units[$exponent].'ohms';
    }
}
